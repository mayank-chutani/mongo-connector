...
class NodesAndRelationshipsUpdater(object):
  ...
  def run_update(self, update_spec, doc_id, doc_type):
    ...
    for spec in update_spec.keys():
      if spec=='$set':
        ...
        statement = "MATCH (d:Document:`{doc_type}`) WHERE d.uid={{doc_id}} SET d+={{set_parameter}}".format(doc_type=doc_type)
        self.statements_with_params.append({statement: params_dict})
      elif spec=='$unset':
        update_value_list = update_spec['$unset']
        for update_value in update_value_list.keys():
          statement = "MATCH (d:Document:{doc_type}), (c:Document:{update_value}) WHERE d.uid={{doc_id}} OPTIONAL MATCH (d)-[r]-(c) DELETE r WITH d, c OPTIONAL MATCH (c)-[s]-() WITH d,c,s, CASE WHEN s IS NULL THEN c ELSE NULL END AS n DELETE n".format(doc_type=doc_type, update_value=update_value)
          self.statements_with_params.append({statement: params_dict})
          statement = "MATCH (d:Document:`{doc_type}` {{ uid: {{doc_id}} }} ) REMOVE d.`{remove_parameter}` ".format(doc_type=doc_type, remove_parameter=update_value)
          self.statements_with_params.append({statement: params_dict})
      else:
  ...
  def remove_legacy_nodes(self, doc_id, doc_type):
    params_dict = {"doc_id": doc_id}
    statement = "MATCH (d:Document:`{doc_type}`) WHERE d.uid={{doc_id}} OPTIONAL MATCH (d)-[r]-(c) DELETE r WITH d, c OPTIONAL MATCH (c)-[s]-() WITH d,c,s, CASE WHEN s IS NULL THEN c ELSE NULL END AS n DELETE n".format(doc_type=doc_type)
    self.statements_with_params.append({statement: params_dict})

  def clear_node(self, doc_type, doc_id):
    params_dict = {"doc_id": doc_id}
    params_dict.update({"parameters": {"_id": doc_id}})
    statement = "MATCH (d:Document:`{doc_type}`) WHERE d.uid={{doc_id}} SET d={{}} SET d={{parameters}}".format(doc_type=doc_type)
    self.statements_with_params.append({statement: params_dict})
